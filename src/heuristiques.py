...
def repeter_heur_alea(listeObj , capBoites , nbExperiences=1000):
    ...
    Le but est de varier le plus possible les solutions initiales.
    ...
    for i in range(nbExperiences):
        ...
        if f_x < meilleurF_X:
            ...
            meilleurF_X = f_x
        tempsActuel = clock()
        if tempsActuel - tempsDebut >= 60: # Limite de 1 minute par bench dépassé ?
...
if __name__ == '__main__':
    capBoites , listeObj = chargerBench("../instances2/bench_20_15")
    listeObj = trier_objets_aleatoire(listeObj)
    boites1 , capPerdue1 = repeter_heur_alea(listeObj, capBoites)
